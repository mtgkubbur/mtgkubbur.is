...
- `/rotisserie` (unlisted, noindex) — live status page for the Meta Memories rotisserie draft. `scripts/rotisserie/` (`parse.py`, `scryfall.py`) builds `data/kubbur/rotisserie.json` + `rotisserie_cards.json` from the draft-grid Google Sheet and the Scryfall card cache (which always includes the five basic lands for the deckbuilder); `.github/workflows/rotisserie.yml` rebuilds/republishes from the sheet, but is **dispatch-only since 2026-08-10** — the draft is over, so the half-hourly poll was retired (every run since 2026-08-05 skipped the sync). Run it manually if a sheet cell is corrected; the gate still no-ops when nothing changed. The `rotisserie*` excludes in the cube_rankings mirror sync remain **required** — with the poll off, deleted files would not be regenerated automatically. `app/static/js/rotisserie.js` renders player pools, the remaining-pool browser, and the pick log from one fetch of `/data/rotisserie`.
- `/rotisserie/deck` (unlisted, noindex) — deckbuilder on the same `/data/rotisserie` payload: mana-value columns with creature/non-creature piles (manual MV filing via drag or popover picker — `cmc_overrides`), sideboard, basic-land steppers + typed counts, speculative picks from the remaining pool, hover card preview, undo (Ctrl/Cmd+Z), sample-hand dealer, 40-card/land-count target hints, and read-only share links (`#share=` URL fragment, zero backend). All user state in localStorage (`rot-deck:<player>`); the deck is derived (live pool + speculative − sideboard), so new picks appear on reload. `app/static/js/rotisserie-deck.js` (versioned `?v=N` in `rotisserie_deck.html`, same bump convention as above); unlisted policy: public pages link to neither rotisserie page, the pair may interlink. Specs: `docs/superpowers/specs/2026-08-04-rotisserie-deckbuilder-design.md`, `2026-08-05-mana-odds-karsten-redesign.md` (Karsten-2022 conditional castability, cache v4 `fetch_basics_only`), `2026-08-05-deckbuilder-qol-batch.md` (QoL batch + joint multicolour castability via Hall's-condition hypergeometric, oracle-verified).
